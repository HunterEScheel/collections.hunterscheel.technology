...
export async function joinQuest(
  ...
  const { error } = await supabase.rpc("hexmap_join_quest", {
    p_quest_id: questId,
...
export async function leaveQuest(
  ...
  const { error } = await supabase.rpc("hexmap_leave_quest", {
    p_quest_id: questId,
    p_player_name: playerName,
  });
...
export async function setQuestActive(
  ...
  const { error } = await supabase.rpc("hexmap_set_quest_active", {
    p_quest_id: questId,
    p_player_name: playerName,
  });
...
export async function addInitiativeEntry(
  ...
  await supabase.from("hexmap_initiative_tracker").insert({
    name,
    initiative,
...
export async function saveCharacter(
  ...
  const { error } = await supabase.rpc("hexmap_save_character", {
    p_old_name: oldName,
...
export async function createQuestFinding(
  ...
  const { data, error } = await supabase.rpc("hexmap_create_quest_finding", {
    p_quest_id: questId,
    p_author: author,
    p_hex_col: hexCol,
    p_hex_row: hexRow,
...
export async function deleteQuestFinding(id: string): Promise<void> {
  await supabase.from("hexmap_quest_findings").delete().eq("id", id);
}
