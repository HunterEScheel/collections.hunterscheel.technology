...
export async function purchaseItem(
  ...
  const { error } = await supabase.rpc("hexmap_purchase_shop_item", {
    p_id: id,
    p_buyer: buyer ?? null,
  });
...
export async function purchaseEquipment(
  ...
  const { error } = await supabase.rpc("hexmap_purchase_equipment", {
    p_buyer: buyer,
...
export async function sellPurchase(
  ...
  const { data, error } = await supabase.rpc("hexmap_sell_purchase", {
    p_id: id,
    p_player: player,
  });
...
export async function disposePurchase(
  ...
  const { error } = await supabase.rpc("hexmap_dispose_purchase", {
    p_id: id,
    p_player: player,
  });
